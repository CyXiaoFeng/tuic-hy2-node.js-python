...
TUNNEL_CONFIG="cloudflare_tunnel_config.json"
TUNNEL_LINK="cloudflare_tunnel_link.txt"
...
EOF
  echo "Cloudflare Tunnel config generated at: $TUNNEL_CONFIG"
}
...
gen_tunnel_link() {
  ...
  cat > "$TUNNEL_LINK" <<EOF
vless://$VLESS_UUID@$TUNNEL_DOMAIN:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$MASQ_DOMAIN&fp=chrome&pbk=$pub&sid=$sid&type=tcp&spx=/#VLESS-Reality-Tunnel
EOF

  echo "========================================="
  echo "VLESS + TCP + Reality Node (with Cloudflare Tunnel):"
  cat "$TUNNEL_LINK"
  echo "========================================="
}
...
# ========== 主函数 ==========
main() {
  echo "Deploying VLESS + TCP + Reality (Single Node) + Cloudflare Tunnel"
